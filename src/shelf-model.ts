...
export interface RodModification {
  ...
  direction?: 'up' | 'down'; // For extends: direction of extension
}
...
export interface PlateSegmentResult {
  sku_id: number;
  rodIds: number[]; // Fully resolved rod IDs (no -1 placeholders)
  y: number;
  action: PlateAction;
...
export function canAddPlateSegment(rodId: number, y: number, direction: Direction, shelf: Shelf): PlateSegmentResult | undefined {
  ...
  if (sourcePlateId !== undefined) {
    ...
    const rodPlan = validateRodCreation({ x: targetX, y: y }, shelf);
    ...
    const newConnections = direction === 'left'
      ...
      : [...existingPlate.connections, newRodId];
    ...
    return {
      ...
      action: 'extend',
      existingPlateId: sourcePlateId,
      rodCreationPlan: rodPlan,
      segmentWidth: defaultPlateGapWidth
    };
  }

  // Validate rod creation for new plate
  const rodPlan = validateRodCreation({ x: targetX, y: y }, shelf);
  ...
  return {
    ...
    rodIds: direction === 'left' ? [newRodId, rodId] : [rodId, newRodId],
    y: y,
    action: 'create',
    rodCreationPlan: rodPlan,
    segmentWidth: defaultPlateGapWidth
...
/**
 * Converts PlateSegmentResult into rod modifications using pre-validated plans.
 * Returns rodModifications array. Connections are already resolved in the result.
 */
function prepareRodModifications(
  result: PlateSegmentResult,
  shelf: Shelf
): RodModification[] {
  const rodModifications: RodModification[] = [];

  // Process rod creation plan (if present)
  if (result.rodCreationPlan) {
    ...
    rodModifications.push({
      ...
      newSkuId: plan.action === 'create' ? plan.newSkuId : plan.action === 'merge' ? plan.mergedSkuId : plan.extendedSkuId,
...
/**
 * Helper to create rod modifications from extension map
 */
function createExtensionRodModifications(
  extension: Map<number, { newSKU_id: number, spanToAdd: number }>,
  direction: 'up' | 'down',
  shelf: Shelf
): RodModification[] {
  const rodModifications: RodModification[] = [];

  for (const [extRodId, ext] of extension.entries()) {
...
/**
 * Helper to try adding a rod extension ghost plate (up or down)
 */
function tryAddRodExtensionGhost(
  rodId: number,
  rod: Rod,
  rightRodId: number,
  rightRod: Rod,
...
/**
 * Helper to add a directional ghost plate (left or right)
 */
function tryAddDirectionalGhost(
  rodId: number,
  rod: Rod,
  y: number,
  direction: Direction,
  ...
  const result = canAddPlateSegment(rodId, y, direction, shelf);
  const defaultWidth = 600;
  const directionMultiplier = direction === Direction.Left ? -1 : 1;
  ...
  if (result) {
    ...
    candidate = {
      ...
      width: segmentWidth,
      rodModifications: rodModifications.length > 0 ? rodModifications : undefined
    };
  } else {
    candidate = {
      midpointPosition: {
        x: rod.position.x + (directionMultiplier * defaultWidth / 2),
        y: y
      },
      legal: false,
      direction: direction === Direction.Left ? 'left' : 'right',
...
export function regenerateGhostPlates(shelf: Shelf): void {
  ...
  for (let [rodId, rod] of shelf.rods) {
    ...
    for (const ap of rod.attachmentPoints) {
      ...
      if (!hasLeftPlate) {
        tryAddDirectionalGhost(rodId, rod, y, Direction.Left, shelf);
      }

      if (!hasRightPlate) {
        tryAddDirectionalGhost(rodId, rod, y, Direction.Right, shelf);
      }
    }
