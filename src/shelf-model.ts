...
export function addOrExtendRod(position: Vec2f, shelf: Shelf): number {
  ...
  // Find all rods at the same X position
  const rodsAtX: { rodId: number; rod: Rod; bottomY: number; topY: number }[] = [];
  for (const [rodId, rod] of shelf.rods) {
    if (rod.position.x === targetX) {
      const rodSKU = AVAILABLE_RODS.find(r => r.sku_id === rod.sku_id);
      if (!rodSKU) continue;
      const positions = calculateAttachmentPositions(rodSKU);
      const bottomY = rod.position.y + positions[0];
      const topY = rod.position.y + positions[positions.length - 1];
      rodsAtX.push({ rodId, rod, bottomY, topY });
    }
  }

  // Sort by Y position (bottom to top)
  rodsAtX.sort((a, b) => a.bottomY - b.bottomY);

  // Find rod immediately below (top of rod is close to targetY)
  let rodBelow: typeof rodsAtX[0] | undefined;
  for (const r of rodsAtX) {
    if (r.topY < targetY) {
      rodBelow = r;
    }
  }

  // Find rod immediately above (bottom of rod is close to targetY)
  let rodAbove: typeof rodsAtX[0] | undefined;
  for (const r of rodsAtX) {
    if (r.bottomY > targetY) {
      rodAbove = r;
      break;
    }
  }

  // Try to extend the rod below upward to reach targetY
  if (rodBelow) {
    const gapToTarget = targetY - rodBelow.topY;
    for (const span of [200, 300]) {
      if (Math.abs(gapToTarget - span) < 1) {
        const extended = tryExtendRodUpward(rodBelow.rodId, span, shelf);
        if (extended) {
          // If there's also a rod above, try to merge them
          if (rodAbove) {
            const gapToAbove = rodAbove.bottomY - targetY;
            for (const spanAbove of [200, 300]) {
              if (Math.abs(gapToAbove - spanAbove) < 1) {
                const merged = tryMergeRodsVertically(rodBelow.rodId, rodAbove.rodId, spanAbove, shelf);
                if (merged) {
  ...
  // Try to extend the rod above downward to reach targetY
  if (rodAbove) {
    const gapToTarget = rodAbove.bottomY - targetY;
    for (const span of [200, 300]) {
      if (Math.abs(gapToTarget - span) < 1) {
        const extended = tryExtendRodDownward(rodAbove.rodId, span, shelf);
        if (extended) {
          return rodAbove.rodId;
        }
      }
    }
  }
...
// Try to extend a rod upward by adding a span at the top
function tryExtendRodUpward(rodId: number, spanToAdd: number, shelf: Shelf): boolean {
  ...
  // Find SKU with additional span at the end
  ...
  const newRelativeY = positions[positions.length - 1];
  rod.attachmentPoints.push({ y: newRelativeY });

  return true;
}

// Try to extend a rod downward by adding a span at the bottom
function tryExtendRodDownward(rodId: number, spanToAdd: number, shelf: Shelf): boolean {
  const rod = shelf.rods.get(rodId);
  if (!rod) return false;
  ...
  // Find SKU with additional span at the beginning
  const newSKU = AVAILABLE_RODS.find(sku => {
    if (sku.spans.length !== rodSKU.spans.length + 1) return false;
  ...
  // Adjust rod position downward
  rod.position.y -= spanToAdd;
  rod.sku_id = newSKU.sku_id;

  // Rebuild attachment points
  const positions = calculateAttachmentPositions(newSKU);
  rod.attachmentPoints = positions.map(y => ({ y }));

  // Re-attach existing plates
...
// Try to merge two rods vertically (rodBelow extended to include rodAbove)
function tryMergeRodsVertically(rodBelowId: number, rodAboveId: number, gapSpan: number, shelf: Shelf): boolean {
  const rodBelow = shelf.rods.get(rodBelowId);
  const rodAbove = shelf.rods.get(rodAboveId);
  if (!rodBelow || !rodAbove) return false;

  const belowSKU = AVAILABLE_RODS.find(r => r.sku_id === rodBelow.sku_id);
  const aboveSKU = AVAILABLE_RODS.find(r => r.sku_id === rodAbove.sku_id);
  if (!belowSKU || !aboveSKU) return false;

  // Combined spans: belowSKU.spans + gapSpan + aboveSKU.spans
  const combinedSpans = [...belowSKU.spans, gapSpan, ...aboveSKU.spans];

  // Find matching SKU
  const mergedSKU = AVAILABLE_RODS.find(sku => {
    if (sku.spans.length !== combinedSpans.length) return false;
    return sku.spans.every((span, i) => span === combinedSpans[i]);
  });

  if (!mergedSKU) return false;

  // Update the below rod to use the merged SKU
  rodBelow.sku_id = mergedSKU.sku_id;

  // Rebuild attachment points
  const positions = calculateAttachmentPositions(mergedSKU);
  rodBelow.attachmentPoints = positions.map(y => ({ y }));

  // Re-attach plates from the below rod
  shelf.plates.forEach((plate, plateId) => {
    if (plate.connections.includes(rodBelowId)) {
      const attachmentY = plate.y - rodBelow.position.y;
      const attachmentIndex = rodBelow.attachmentPoints.findIndex(ap => ap.y === attachmentY);
      if (attachmentIndex !== -1) {
        rodBelow.attachmentPoints[attachmentIndex].plateId = plateId;
      }
    }
  });

  // Re-attach plates from the above rod (updating their connection to point to below rod)
  shelf.plates.forEach((plate, plateId) => {
    if (plate.connections.includes(rodAboveId)) {
      // Update connection to point to the merged rod
      const connectionIndex = plate.connections.indexOf(rodAboveId);
      plate.connections[connectionIndex] = rodBelowId;
  ...
  // Remove the above rod
  shelf.rods.delete(rodAboveId);
