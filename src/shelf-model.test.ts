...
testGroup('Ghost Plate Generation - Rod Creation', () => {
  test('Simple rod creation at empty position generates correct ghost', () => {
    ...
    // Should have ghosts for extending plate left and right, which would create new rods
    const ghosts = shelf.ghostPlates.filter(g => g.legal && g.action === 'create');
    assertTrue(ghosts.length > 0, 'Should have at least one rod creation ghost');
